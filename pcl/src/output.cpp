...
//默认就是而二进制块读取转换为模块化的PointCLoud格式里pcl::PointXYZ作为点类型  然后打印出来
  ...
  for (size_t i = 0; i < cloud->points.size (); ++i){
	...
	//v_y.push_back(a[i]); 
	if(y==0.0 && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x <6){
...
if(y==round(0.2*y_r) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x <6){
	//std::cout  <<" cloud->points[i].z:   " << cloud->points[i].z<< std::endl;
		if(cloud->points[i].z> 0)
...
if(y==round(0.4*y_r) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x <6){
		if(cloud->points[i].z> 0)
...
if(y==round(0.6*y_r) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x <6){
		if(cloud->points[i].z> 0)
...
if(y==round(0.8*y_r) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x < 6){
		if(cloud->points[i].z> 0)
...
if(y==-round(0.2*y_l) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x < 6){
		if(cloud->points[i].z> 0)
...
if(y==-round(0.4*y_l) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x < 6){
		if(cloud->points[i].z> 0)
...
if(y==-round(0.6*y_l) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x < 6){
		if(cloud->points[i].z> 0)
...
if(y==-round(0.8*y_l) && fabs(cloud->points[i].z) >0.05 && cloud->points[i].x < 6){
		if(cloud->points[i].z> 0)
...
//std::cout << 0 <<" dd:   " << lp4[0]<< std::endl;
//std::cout << 0 <<" (accumulate(lp4.begin(),lp4.end(),0)):   " << (accumulate(lp4.begin(),lp4.end(),0))<< std::endl;
...
std::cout  <<"Height distribution of tunnel section:   " << std::endl;
